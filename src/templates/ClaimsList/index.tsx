...
export default function ClaimsList(props) {
  ...
  function getClaims() {
    ...
    data.forEach((item) => {
      const claimItem = {
        claimRef: item.pyID,
        dateCreated: DateFormatter.Date(item.pxCreateDateTime, {
          format: 'DD/MM/YYYY',
        }),
        children: [],
        actionButton: (
      ...
      if (item.Claim.ChildrenJSON) {
        ...
        claimItem.children.push({
          ...
          dob: item.Claim.Child.DateOfBirth
            ? GBdate(item.Claim.Child.DateOfBirth)
            : '',
        });
      }
        ...
        >
          <div className="govuk-summary-list__row">
              ...
              {claimItem.children.map((child) => (
                <p key={child.firstName}>
                  ...
                  {child.dob && (
                    <>
                      <br />
                      <span className="govuk-!-font-weight-regular">{`${t(
                        'DATE_OF_BIRTH'
                      )} ${child.dob}`}</span>
                      <br />
                    </>
                  ...
                  <span className="govuk-!-font-weight-regular">
                    {t('CREATED_DATE')} {claimItem.dateCreated}
                  </span>
                </p>
              ))}
            ...
            <dd className="govuk-summary-list__actions govuk-!-width-one-half">
              <strong
                className={`govuk-tag govuk-tag--${claimItem.status.tagColour}`}
              >
