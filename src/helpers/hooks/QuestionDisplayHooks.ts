...
export default function useIsOnlyField(
  ...
  useEffect(() => {
    if (
      PCore.getStoreValue('displayAsSingleQuestion', '', 'app') &&
      DFContext.DFName === -1
    ) {
    ...
    // Otherwise, use the Assignment context's singleQuestion page value, to fall back to the original logic (checking number of editable fields);
      ...
      const pageHasNonEditableField = PCore.getStoreValue(
        'pageHasNonEdtiableField',
        '',
        'app'
      );
      ...
      const editableFieldsCount = PCore.getFormUtils().getEditableFields(
        PCore.getContainerUtils().getActiveContainerItemContext(
          `${context}/workarea`
        )
      ).length;
...
function registerNonEditableField(nonEditableCondition = true) {
  useEffect(() => {
    if (
      ...
      !PCore.getStoreValue('pageHasNonEdtiableField', '', 'app')
    ) {
